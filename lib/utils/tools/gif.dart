/*
 * @Description  : gif管理器
 * @Date         : 2021-07-02 11:24:20
 * @Author       : 保锋
 * @LastEditTime : 2021-07-02 14:22:42
 * @LastEditors  : 保锋
 * @FilePath     : \knowledge_contest\lib\utils\tools\gif.dart
 */
...
class GifImageState extends State<GifImage> {
  ...
  void didUpdateWidget(GifImage oldWidget) {
    ...
    if (widget.image != oldWidget.image) {
      fetchGif(widget.image).then((List<ImageInfo> imageInfors) {
        if (mounted)
          setState(() {
            _infos = imageInfors;
            _fetchComplete = true;
...
Future<List<ImageInfo>> fetchGif(ImageProvider provider) async {
  List<ImageInfo> infos = <ImageInfo>[];
  dynamic data;
